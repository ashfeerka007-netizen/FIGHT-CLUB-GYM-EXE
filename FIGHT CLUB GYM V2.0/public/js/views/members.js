...
const MembersView = {
        ...
        <div class="modal-card modal-lg">
          ...
          <div class="modal-body">
            <form id="member-registration-form" enctype="multipart/form-data">
              ...
              <input type="hidden" id="reg-fingerprint-template">
              <input type="hidden" id="reg-fingerprint-image">
              <input type="hidden" id="reg-fingerprint-quality" value="0">
              ...
              <div class="grid-2">
                ...
                <div>
                  ...
                  <!-- Mantra Biometric Fingerprint Section -->
                  <div class="form-group" style="background: rgba(0, 0, 0, 0.25); border: 1px dashed var(--color-border); border-radius: var(--radius-sm); padding: 10px 12px; margin-top: 10px;">
                    <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom: 6px;">
                      <span style="font-size:0.875rem; font-weight:600;"><i data-lucide="fingerprint" style="width:14px; height:14px; display:inline-block; vertical-align:middle; margin-right:4px;"></i> Mantra Fingerprint (MFS100)</span>
                      <span id="reg-fingerprint-badge" class="badge" style="display:none; font-size:0.7rem; font-weight:700;"></span>
                    </div>
                    <div style="display: flex; gap: 10px; align-items: center;">
                      <div id="reg-fingerprint-box" style="width: 48px; height: 58px; background: rgba(255,255,255,0.03); border: 1px dashed rgba(255,255,255,0.2); border-radius: 4px; display: flex; align-items: center; justify-content: center; overflow: hidden; flex-shrink: 0;">
                        <img id="reg-fingerprint-img" src="" alt="Fingerprint" style="width:100%; height:100%; object-fit:contain; display:none;">
                        <i id="reg-fingerprint-icon" data-lucide="fingerprint" style="width: 24px; height: 24px; opacity:0.35;"></i>
                      </div>
                      <div style="flex: 1;">
                        <button type="button" class="btn btn-secondary btn-sm" id="btn-scan-reg-fingerprint" style="width: 100%; font-size: 0.8rem; padding: 6px 10px;">
                          <i data-lucide="scan"></i> Scan on Mantra MFS100
                        </button>
                        <div id="reg-fingerprint-status" style="font-size: 0.72rem; color: var(--color-text-muted); margin-top: 4px;">
                          Click to scan right thumb on sensor
                        </div>
                      </div>
                    </div>
                  </div>
  ...
  bindEvents: () => {
    ...
    addBtn.addEventListener('click', () => {
      ...
      document.getElementById('reg-fingerprint-template').value = '';
      document.getElementById('reg-fingerprint-image').value = '';
      document.getElementById('reg-fingerprint-quality').value = '0';
      const fpImg = document.getElementById('reg-fingerprint-img');
      const fpIcon = document.getElementById('reg-fingerprint-icon');
      const fpBadge = document.getElementById('reg-fingerprint-badge');
      const fpStatus = document.getElementById('reg-fingerprint-status');
      if (fpImg) fpImg.style.display = 'none';
      if (fpIcon) fpIcon.style.display = 'block';
      if (fpBadge) fpBadge.style.display = 'none';
      if (fpStatus) fpStatus.textContent = 'Click to scan right thumb on sensor';
    ...
    // Mantra Fingerprint Scan in Member Registration/Edit Form
    const scanRegFpBtn = document.getElementById('btn-scan-reg-fingerprint');
    if (scanRegFpBtn) {
      scanRegFpBtn.addEventListener('click', async () => {
        const templateInput = document.getElementById('reg-fingerprint-template');
        const imageInput = document.getElementById('reg-fingerprint-image');
        const qualityInput = document.getElementById('reg-fingerprint-quality');
        const fpBadge = document.getElementById('reg-fingerprint-badge');
        const fpImg = document.getElementById('reg-fingerprint-img');
        const fpIcon = document.getElementById('reg-fingerprint-icon');
        const fpStatus = document.getElementById('reg-fingerprint-status');

        scanRegFpBtn.disabled = true;
        scanRegFpBtn.innerHTML = '<i data-lucide="loader"></i> Scanning...';
        fpStatus.innerHTML = '<span style="color: var(--color-accent); font-weight: 600;">🔴 Red light ON. Place member finger firmly on scanner...</span>';
        lucide.createIcons();

        try {
          const capture = await mantraClient.captureFingerprint({ quality: 50, timeout: 10 });
          if (capture.success) {
            templateInput.value = capture.isoTemplate || '';
            imageInput.value = capture.bitmapData || '';
            qualityInput.value = capture.quality || 0;

            if (capture.bitmapData) {
              fpImg.src = `data:image/bmp;base64,${capture.bitmapData}`;
              fpImg.style.display = 'block';
              fpIcon.style.display = 'none';
            }

            fpBadge.textContent = `✓ ${capture.quality}% Quality`;
            fpBadge.style.background = 'rgba(76, 175, 80, 0.2)';
            fpBadge.style.color = '#4caf50';
            fpBadge.style.display = 'inline-block';
            fpStatus.innerHTML = '<span style="color: #4caf50; font-weight: 600;">✓ Scanned successfully!</span>';
            showToast(`Fingerprint captured (${capture.quality}% quality)!`, 'success');
          } else {
            fpStatus.innerHTML = `<span style="color: #f44336;">⚠️ ${capture.errorDescription}</span>`;
            showToast(capture.errorDescription, 'error');
          }
        } catch (err) {
          fpStatus.innerHTML = `<span style="color: #f44336;">Scan error: ${err.message}</span>`;
          showToast(err.message, 'error');
        } finally {
          scanRegFpBtn.disabled = false;
          scanRegFpBtn.innerHTML = '<i data-lucide="scan"></i> Scan on Mantra MFS100';
          lucide.createIcons();
        }
      });
    }
    ...
    form.addEventListener('submit', async (e) => {
      ...
      formData.append('fingerprint_template', document.getElementById('reg-fingerprint-template')?.value || '');
      formData.append('fingerprint_image', document.getElementById('reg-fingerprint-image')?.value || '');
      formData.append('fingerprint_quality', document.getElementById('reg-fingerprint-quality')?.value || '0');
  ...
  showEditForm: async (id) => {
    ...
    // Populate biometric fingerprint preview
    const templateInput = document.getElementById('reg-fingerprint-template');
    const imageInput = document.getElementById('reg-fingerprint-image');
    const qualityInput = document.getElementById('reg-fingerprint-quality');
    const fpBadge = document.getElementById('reg-fingerprint-badge');
    ...
    if (templateInput) templateInput.value = m.fingerprint_template || '';
    if (imageInput) imageInput.value = m.fingerprint_image || '';
    if (qualityInput) qualityInput.value = m.fingerprint_quality || 0;

    if (m.fingerprint_image || m.fingerprint_template) {
      if (m.fingerprint_image) {
        fpImg.src = m.fingerprint_image.startsWith('data:') ? m.fingerprint_image : `data:image/bmp;base64,${m.fingerprint_image}`;
        fpImg.style.display = 'block';
        if (fpIcon) fpIcon.style.display = 'none';
      }
      if (fpBadge) {
        fpBadge.textContent = `${m.fingerprint_quality || 100}% Enrolled`;
        fpBadge.style.background = 'rgba(76, 175, 80, 0.2)';
        fpBadge.style.color = '#4caf50';
        fpBadge.style.display = 'inline-block';
      }
      if (fpStatus) {
        fpStatus.innerHTML = '<span style="color: #4caf50; font-weight: 600;">✓ Biometric enrolled (Click scan to update)</span>';
      }
    } else {
      if (fpImg) fpImg.style.display = 'none';
      if (fpIcon) fpIcon.style.display = 'block';
